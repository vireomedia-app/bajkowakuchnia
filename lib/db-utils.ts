...
export async function createProduct(data: {
  ...
  try {
    ...
    // Determine packaging type (default to 'bulk')
    const packagingType = data.packagingType || 'bulk'
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      const product = await tx.product.create({
        data: {
          ...
          packageWeight: isPackaged ? (data.packageWeight ?? null) : null,
          packageUnit: isPackaged && data.packageWeight ? (data.packageUnit || 'g') : null,
