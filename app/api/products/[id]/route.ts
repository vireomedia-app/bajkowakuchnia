...
export async function PATCH(
  ...
  try {
    ...
    // Validate unit
    const validUnits = ['g', 'kg', 'ml', 'l', 'szt']
    if (!validUnits.includes(unit)) {
      return NextResponse.json(
        { error: 'Jednostka miary musi być jedną z: g, kg, ml, l, szt' },
        { status: 400 }
      )
    }
    ...
    // Determine packaging type (default to 'bulk' for existing products without this field)
    const effectivePackagingType = packagingType || 'bulk'
    const isPackaged = effectivePackagingType === 'packaged'
    ...
    const updatedProduct = await prisma.product.update({
      ...
      data: {
        ...
        // Only store packageWeight/packageUnit for packaged products
        packageWeight: isPackaged ? packageWeight : null,
        packageUnit: isPackaged && packageWeight ? (packageUnit || 'g') : null,
        manufacturer,
        calories,
        salt,
        protein,
