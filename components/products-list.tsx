...
import { Package, TrendingUp, TrendingDown, AlertTriangle, ArrowUp, ArrowDown, Barcode } from 'lucide-react'
...
// Helper function to check if barcode is missing
function isBarcodeEmpty(barcode: string | null | undefined): boolean {
...
function MissingBarcodeIndicator() {
  return (
    <TooltipProvider delayDuration={300}>
      <Tooltip>
        <TooltipTrigger asChild>
          <div className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-xs font-medium bg-red-100 text-red-700 border border-red-300 cursor-help">
            <Barcode className="w-3 h-3" />
            <span className="hidden sm:inline">Brak kodu</span>
          </div>
        </TooltipTrigger>
        <TooltipContent>
          <p>Ten produkt nie ma kodu kreskowego – uzupełnij go.</p>
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
  )
                ...
                <motion.tr 
                  ...
                  onClick={() => {
                    if (isEditMode && onToggleSelect) {
                      if (product?.id) onToggleSelect(product.id)
                    } else {
                ...
                >
                  {isEditMode && (
                    <td className="px-3 py-3" onClick={(e) => e.stopPropagation()}>
                      <Checkbox 
                        checked={isSelected}
                        onCheckedChange={() => (product?.id ? onToggleSelect?.(product.id) : undefined)}
                      />
                    </td>
                  )}
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className={`text-sm text-gray-900 ${hasAllergens ? 'font-bold' : 'font-medium'}`} style={{ color: '#111827' }}>
                          ...
                          {isBarcodeEmpty(product?.barcode) && <MissingBarcodeIndicator />}
                        </div>
                      </div>
                    </div>
                  </td>
