...
import { Package, TrendingUp, TrendingDown, Calendar, AlertTriangle, Barcode } from 'lucide-react'
import { EditProductButton } from './edit-product-button'
import { DeleteProductButton } from './delete-product-button'
import { FillFromLeclercButton } from './fill-from-leclerc-button'
...
export function ProductHeader({ product }: ProductHeaderProps) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md border border-gray-100 overflow-hidden">
      ...
      <div className="p-6">
        {/* Warning banner for missing barcode */}
        {isBarcodeEmpty(product?.barcode) && (
          <div className="mb-6 flex items-center gap-3 p-4 bg-amber-50 border border-amber-200 rounded-lg">
            ...
            <div>
              <p className="text-sm font-medium text-amber-800">
                Ten produkt nie ma kodu kreskowego
              </p>
              <p className="text-xs text-amber-700 mt-0.5">
                Część funkcji (np. automatyczne uzupełnianie danych z Leclerc) będzie niedostępna.
                Uzupełnij kod kreskowy w edycji produktu.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="mt-6 pt-6 border-t border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900 mb-2">
            Wartości odżywcze (na {product.unit === 'szt' ? 'sztukę' : product.unit === 'ml' || product.unit === 'l' ? '100 ml' : '100 g'})
          </h3>
          ...
          {/* Fill from Leclerc button - shows disabled state if no barcode */}
          <div className="mt-4 pt-4 border-t border-gray-200">
            <FillFromLeclercButton 
