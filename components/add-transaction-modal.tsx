...
interface AddTransactionModalProps {
  ...
  productId: string
  productUnit: string
  productPackagingType?: 'bulk' | 'packaged' | string | null
  productPackageWeight?: number | null
  productPackageUnit?: string | null
}

export function AddTransactionModal({ isOpen, onClose, productId, productUnit, productPackagingType, productPackageWeight, productPackageUnit }: AddTransactionModalProps) {
  // Check if product is packaged (has package info)
  const isPackaged = productPackagingType === 'packaged' && productPackageWeight && productPackageWeight > 0
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const rawQuantity = parseFloat(formData.quantity)
      
      // Przelicz wagę opakowania na jednostkę bazową (kg lub l)
      const getPackageWeightInBaseUnit = (): number => {
        if (!productPackageWeight) return 0
        const pkgUnit = productPackageUnit || 'g'
        // Convert to base unit (kg or l)
        if (pkgUnit === 'g' && (productUnit === 'kg' || productUnit === 'g')) return productPackageWeight / 1000
        if (pkgUnit === 'ml' && (productUnit === 'l' || productUnit === 'ml')) return productPackageWeight / 1000
        if (pkgUnit === 'kg' && productUnit === 'g') return productPackageWeight * 1000
        if (pkgUnit === 'l' && productUnit === 'ml') return productPackageWeight * 1000
        return productPackageWeight
      }
      
      const effectiveQuantity =
        usePackages && isPackaged
          ? rawQuantity * getPackageWeightInBaseUnit()
          : rawQuantity
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="quantity">
              Ilość ({usePackages ? 'opakowań' : productUnit})
            </Label>
            {isPackaged && (
              <div className="flex items-center gap-2 text-xs text-gray-600 mb-1">
                <input
                  ...
                  disabled={isLoading}
                />
                <label htmlFor="usePackages" className="cursor-pointer">
                  Podaj w liczbie opakowań (1 opak. = {productPackageWeight} {productPackageUnit || 'g'})
                </label>
              </div>
            )}
            <Input
            ...
            <p className="text-xs text-gray-500">
              {usePackages && isPackaged
                ? `Podaj liczbę opakowań. System przeliczy ją automatycznie na ${productUnit}.`
                : `Podaj ilość w ${
                    productUnit === 'kg'
                      ? 'kilogramach'
                      : productUnit === 'g'
